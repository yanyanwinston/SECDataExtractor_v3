    ...
    try:
        ...
        if not arelle_processor.check_arelle_available():
            ...
            if not arelle_processor.install_arelle():
                raise RuntimeError("Failed to install Arelle. Please install manually: pip install arelle")
        ...
        if args.verbose:
            ...
            for statement in result.statements:
                print(f"    - {statement.name}: {len(statement.periods)} periods, {len(statement.rows)} rows")
