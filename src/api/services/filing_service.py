"""Service responsible for filing discovery, caching, and downloads."""
...
from .constants import _ALLOWED_FORM_TYPES
from .exceptions import DataRetrievalError
from .utils import normalize_date_bounds, parse_datetime
...
class FilingRetrievalService:
    """Handles filing discovery, caching, and download management."""
    ...
    def __init__(
        ...
        self._temp_root = Path(tempfile.gettempdir()) / "sec_data_api"
        self._temp_root.mkdir(exist_ok=True)
        self._download_listeners: List[Callable[[str], None]] = []
    ...
    def ensure_local_filing(self, filing: FilingRecord) -> Optional[FilingRecord]:
        if filing.available_locally():
            return filing

        logger.info(
            "Filing %s not cached locally; attempting download",
            filing.accession_number,
        )
        return self._download_filing_record(filing)

    @property
    def temp_root(self) -> Path:
        return self._temp_root

    def register_download_listener(self, callback: Callable[[str], None]) -> None:
        if callback not in self._download_listeners:
            self._download_listeners.append(callback)
    ...
    def _record_from_metadata(self, metadata_path: Path, source: str) -> FilingRecord:
        ...
        filing_date = parse_datetime(filing_info.get("filing_date"))
        report_date = parse_datetime(filing_info.get("report_date"))
    ...
    def _ensure_cached(
        ...
        if cached_records:
            ...
            self._notify_download_listeners(ticker)
        return cached_records

    def _download_filing_record(self, record: FilingRecord) -> Optional[FilingRecord]:
        if not record.cik:
            logger.warning(
                "Cannot download filing %s because CIK is missing",
        ...
        try:
            ...
            logger.warning(
                "Download failed for %s: %s", record.accession_number, exc
            )
            return None

        if not result.success or not result.metadata_path:
            logger.warning(
                "Download unsuccessful for %s", record.accession_number
            )
            return None

        refreshed = self._record_from_metadata(result.metadata_path, source="downloaded")
        cache_entry = self._cache.get(record.ticker)
        if not cache_entry or self._is_cache_expired(cache_entry.loaded_at):
            cache_entry = _TickerCache(records={}, loaded_at=datetime.utcnow())
            self._cache[record.ticker] = cache_entry
        cache_entry.records[refreshed.accession_number] = refreshed
        cache_entry.loaded_at = datetime.utcnow()
        self._notify_download_listeners(record.ticker)
        return refreshed
    ...
    def _notify_download_listeners(self, ticker: str) -> None:
        for callback in self._download_listeners:
            try:
                callback(ticker)
            except Exception as exc:  # pragma: no cover - defensive guard
                logger.debug("Download listener failed for %s: %s", ticker, exc)
